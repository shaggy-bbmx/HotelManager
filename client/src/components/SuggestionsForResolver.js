...
const SuggestionsForResolver = ({ props }) => {


    const {
        resolvedByEmployeeNo: employeeNo,
    ...
    return (
        <div className='w-full h-full p-4'>
            {loading &&
                <Skeleton className='w-full mb-[2px]' sx={{ height: '2rem' }} variant='rectangular'></Skeleton>}
            ...
            {suggestions &&
                suggestions.map(suggestion =>
                    <div key={suggestion.employeeNo} className='w-full p-2 bg-[#5096FF] text-md text-white mb-[1px] flex flex-row cursor-pointer'
                        onClick={
                            (e) => {
                                handleCheckBoxChangeForResolver(e, suggestion.employeeNo)
                            }}>
...
export default SuggestionsForResolver
