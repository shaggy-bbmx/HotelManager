...
const SuggestionsForInitiator = ({ props }) => {

    const {
        initiatedByEmployeeNo: employeeNo,
            ...
            {suggestions &&
                suggestions.map(suggestion =>
                    <div key={suggestion.employeeNo} className='w-full p-2 bg-[#5096FF] text-md text-white mb-[1px] flex flex-row cursor-pointer'
                        onClick={
                            (e) => {
                                handleCheckBoxChangeForInitiator(e, suggestion.employeeNo)
                            }}>
...
export default SuggestionsForInitiator
